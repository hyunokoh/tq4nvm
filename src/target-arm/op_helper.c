...
uint32_t otp_global_key = 1000;
char input_val[32] = {0};

int original_bit_count = 0;
int aes_bit_count = 0;
int otp_bit_count = 0;
int stream_cipher_cnt = 0;
int block_cipher_cnt = 0;
int rtv_kick_count = 0;
int wi_kick_count = 0;

#define OTP_PERIOD 10

uint32_t OTP_encrypt(uint32_t otp_global_key, uint32_t val, uint32_t addr, uint32_t count){
    uint32_t otp_key;
    char temp[96*2];
    unsigned char buf[32];
    //now = time(0);
    //tm = localtime(&now);
    //sprintf(temp, "%" PRIu32 "%" PRIu32 "%04d%02d%02d%02d%02d%02d", otp_global_key, addr, tm->tm_year + 1900, tm->tm_mon + 1, tm -> tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	uint64_t cycle = perfmodel_getCycle();
	uint64_t period = cycle>>OTP_PERIOD;
	
    sprintf(temp, "%" PRIu32 "%" PRIu64 "%" PRIu32, otp_global_key, period, addr);
    AES_set_encrypt_key(key32, 256, &aes_ks3);
    
    AES_encrypt((unsigned char*)temp, buf, &aes_ks3);
    memcpy(&otp_key, buf, sizeof(int));
    return val ^ otp_key;
}

int getOTPCount(int);
int referHotTable(int);
int updateAllReference();

void HELPER(dcache_access_read)(uint32_t addr){
    //latency simulation
    int lat;
    lat = cachesim_daccess(perfmodel_getSimpleCycle(), addr);
    if(is_sampled){
...
uint64_t oldPeriod = 0;
void HELPER(dcache_access_write)(uint32_t new_val, uint32_t old_val, uint32_t addr){
    int original_diffVal, aes_diffVal, otp_diffVal;
    int i;
    uint32_t aes_new_val, aes_old_val, otp_new_val, otp_old_val;
    unsigned char buf[32];
    int otp_old_count,otp_new_count;

	uint64_t newPeriod = perfmodel_getCycle()>>OTP_PERIOD;
    ...
    //latency simulation
    int lat;
    otp_old_count = getOTPCount(addr);
	int old_hot = isHot(addr);
    lat = cachesim_daccess_write(perfmodel_getSimpleCycle(), addr);
	referHotTable(addr);
	int new_hot = isHot(addr);
    otp_new_count = getOTPCount(addr);

	if(new_hot) stream_cipher_cnt++;
	else block_cipher_cnt++;
    ...
    memcpy(input_val, &new_val, sizeof(int));
    AES_encrypt((unsigned char*)input_val, buf, &aes_ks3);
    memcpy(&aes_new_val, buf, sizeof(int));

    memcpy(input_val, &old_val, sizeof(int));
    AES_encrypt((unsigned char*)input_val, buf, &aes_ks3);
    memcpy(&aes_old_val, buf, sizeof(int));

	if(old_hot) 
    		otp_old_val = OTP_encrypt(otp_global_key, old_val, addr, otp_old_count);
	else	otp_old_val = aes_old_val;
	if(new_hot)
    		otp_new_val = OTP_encrypt(otp_global_key, new_val, addr, otp_new_count);
	else	otp_new_val = aes_new_val;

    original_diffVal = new_val ^ old_val;
    aes_diffVal = aes_new_val ^ aes_old_val;
    otp_diffVal = otp_new_val ^ otp_old_val;

    for(i = 0; i < 32; i++){
        original_bit_count += (original_diffVal >> i) & 1;
        aes_bit_count += (aes_diffVal >> i) & 1;
        otp_bit_count += (otp_diffVal >> i) & 1;
    }

	if(aes_bit_count > (1<<24)) {
...
/*
    printf("*************************************************\n");
    printf("original bit_change: %d\n", original_bit_count);
    printf("new value: %x\told value:%x\taddr: %x\n", new_val, old_val, addr);
    printf("=================================================\n");
    printf("AES bit_change: %d\n", aes_bit_count);
    printf("new value: %x\told value:%x\taddr: %x\n", aes_new_val, aes_old_val, addr);
    printf("=================================================\n");
    printf("OTP bit_change: %d\n", otp_bit_count);
    printf("new value: %x\told value:%x\taddr: %x\n", otp_new_val, otp_old_val, addr);
    printf("*************************************************\n");
*/
}
